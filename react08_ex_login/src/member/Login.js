...
function Login(){
    ...
    // form의 데이터(아이디, 비밀번호)를 담을 hook변수
    ...
    //                      방금 이벤트가 발생한 객체 (event변수 이름은 다른 것으로도 변경가능)
    ...
    const loginSubmit = (event)=>{
        ...
        // 아이디 정규표현식 유효성 검사하기
        // 아이디: 영어 대소문자, _, 6~12자까지 허용
        const useridReg = /^[a-zA-Z0-9_]{6,12}$/;   //  같은 의미 => /^\w[6,12]$/
        if( !useridReg.test(logFormData.userid)){
            setLogIdErrorMessage("영어 대소문자, _, 6~12자까지 가능합니다.");
            return false;
        }
        // 비밀번호 입력유무 확인
        ...
        <div className='loginFormCenter'>
            ...
            <form onSubmit={loginSubmit}>
                <div className="form-group"> 
                    <label>아이디</label>
                    <input type="text" name="userid" id="userid" placeholder="아이디 입력"className='form-control' onChange={loginDataChange}/>
                    <div style={divHeight}>{logIdErrorMessage}</div>
                </div>
                <div className='form-group'>
                    <label>비밀번호</label>
                    <input type="password" name="userpwd" id="userpwd" placeholder="비밀번호 입력" className='form-control' onChange={loginDataChange}/>                   
                    <div style={divHeight}>{logPwdErrorMessage}</div>
                </div>
                <input type="submit" value="로그인" className='form-control btn btn-success'/>
            </form>
        </div>
    )
}
